...
export function Loading({ label }: { label?: string }) {
  ...
  return (
    <p className="flex items-center gap-2 py-8 text-sm text-content-muted" role="status">
      <span
        aria-hidden="true"
...
export function Empty({ label }: { label?: string }) {
  const { t } = useTranslation()
  return (
    <p className="panel px-4 py-8 text-center text-sm text-content-muted">
...
export function ErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  return (
    <div className="border-l-2 border-down bg-down-soft px-4 py-4">
